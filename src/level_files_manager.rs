                        ...
                        egui::Frame::popup(ui.style()).show(ui, |ui| {
                            ...
                            let save_index = |loaded_files_index: &[YoleckLevelIndexEntry]| {
                                ...
                                match fs::File::create(&index_file) {
                                    Ok(fd) => {
                                        let index = YoleckLevelIndex::new(
                                            loaded_files_index.iter().cloned(),
                                        );
                            ...
                            if should_list_files {
                                ...
                                let editable_levels_update_result =
                                    fs::read_dir(&levels_directory.0).and_then(|files| {
                                ...
                                loaded_files_index = editable_levels_update_result.and_then(|()| {
                                    ...
                                    files_index
                                        .retain(|file| !existing_files.contains(&file.filename));
                                    ...
                                    egui::ScrollArea::vertical()
                                        ...
                                        .show(ui, |ui| {
                                            for (index, file) in files.iter().enumerate() {
                                                let is_selected =
                                                    if let SelectedLevelFile::Existing(
                                                        selected_name,
                                                    ) = &selected_level_file
                                                    {
